...
@section('content')
    ...
    <div class="container">
        <div class="applications_block">
            <h1 class="header text-center">{{__ ('portfolio.Portfolio.block_02')}}</h1>
            <div class="row">
                @foreach($data as $val)
                    <div class="col-md-10 app_block">
                        <div class="item_block text-center">
                            <img src="{{ asset('images/portfolio/works/'.$val->title_image) }}" alt="">
                            <h2>{{ $val->name }}</h2>
                            <a href="{{ route('portfolio_app_view', $val->id) }}" class="btn btn-more">{{__ ('portfolio.Portfolio.btnText') }} <i class="fa-solid fa-arrow-up-right-from-square"></i></a>
                        </div>
                    </div>
                @endforeach
            </div>
